// =====================================================================================
// COMPONENTE FASE DESPUES - REFACTORIZADO CON SIDE PANEL
// =====================================================================================
...
export class FasedespuesComponent implements OnInit, OnChanges {
  // =====================================================================================
  // INPUTS Y OUTPUTS
  // =====================================================================================
  ...
  readonly FASE = FaseEvaluacion.DESPUES;
  ...
  ngOnInit(): void {
    console.log('🔵 FaseDESPUESComponent ngOnInit', {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    console.log('🟡 FaseDESPUESComponent ngOnChanges', changes);
  ...
  private cargarDetallesPorEvaluacion(): void {
    ...
    this.evaluacionService.obtenerDetallesPorEvaluacion(this.evaluacionId).subscribe({
      next: (detalles) => {
        ...
        const detalleDespues = detalles.find(
          d => d.fase.toUpperCase() === this.FASE
        );
        
        if (detalleDespues) {
          console.log('✅ Detalle DESPUES encontrado:', detalleDespues);
          this.datosFase = detalleDespues;
          this.cargarFotos();
        } else {
          console.warn('⚠️ No se encontró detalle DESPUES');
 ...
 get tipoBadgeFase(): 'success' | 'info' | 'warning' | 'error' | 'default' {
  return 'success'; // DESPUÉS es verde (success)
}
}