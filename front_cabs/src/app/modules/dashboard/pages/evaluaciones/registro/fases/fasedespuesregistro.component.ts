...
import { EvaluacionService } from '../../services/evaluaciones-registro.service';
import { SharedEvaluacionService } from '../../services/shared-evaluacion.service';
import { Foto } from '../../models/evaluacion.dto';
...
export class FaseDespuesRegistroComponent implements OnInit, OnDestroy {
  lugar: string = '';
  ...
  private suscribirACambios(): void {
    ...
    this.sharedService.faseDespuesDetalleId$
      .pipe(takeUntil(this.destroy$))
      .subscribe(detalleId => {
        if (detalleId) {
  ...
  private guardarDatosEnServicio(): void {
    this.sharedService.setFaseDespues({
      lugar: this.lugar,
