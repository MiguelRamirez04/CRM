...
@Component({
  ...
  templateUrl: './cotizaciones-vista.component.html',
...
export class CotizacionesVistaComponent implements OnInit {
  ...
  editarCotizacion(id: number): void {
    console.log('✏️ Editar cotización:', id);
    this.router.navigate(['/dashboard/cotizaciones/editar', id]);
  }

  /**
   * Eliminar cotización
   */
  eliminarCotizacion(id: number, folio: string): void {
    if (!confirm(`¿Está seguro de eliminar la cotización ${folio}?`)) {
      return;
    }
    ...
    this.cotizacionService.eliminar(id).subscribe({
      next: () => {
        console.log('✅ Cotización eliminada exitosamente');
        this.mensajeExito = `Cotización ${folio} eliminada correctamente`;
